...
def resolve_row(question: str, df: pd.DataFrame) -> Optional[pd.Series]:
    ...
    our_ref_col = get_column_name(df, ["Our Reference", "our_reference"])
    if our_ref_col:
    ...
    vendor_col = get_column_name(df, ["Vendor Name", "vendor_name"])
    if vendor_col:
...
def handle_summary(reviewed_df: pd.DataFrame) -> str:
    status_col = get_column_name(reviewed_df, ["review_status", "Review Status"])
    eligibility_col = get_column_name(reviewed_df, ["eligibility_status", "Eligibility Status"])
...
def handle_row_explanation(question: str, reviewed_df: pd.DataFrame) -> str:
    row = resolve_row(question, reviewed_df)
    if row is not None:
        return format_row_explanation(row.to_dict())
    return "Transaction not found / run review first."

def handle_review_queue_summary(reviewed_df: pd.DataFrame) -> str:
    status_col = get_column_name(reviewed_df, ["review_status", "Review Status"])
    eligibility_col = get_column_name(reviewed_df, ["eligibility_status", "Eligibility Status"])
    rationale_col = get_column_name(reviewed_df, ["rationale", "Rationale"])
    confidence_col = get_column_name(reviewed_df, ["confidence_score", "Confidence Score"])
    ...
    samples = []
    ref_col = get_column_name(reviewed_df, ["Trans Ref", "trans_ref"])
    vendor_col = get_column_name(reviewed_df, ["Vendor Name", "vendor_name"])
    amount_col = get_column_name(reviewed_df, ["Amount", "amount"])
...
def handle_location_filter(question: str, reviewed_df: pd.DataFrame) -> str:
    location_col = get_column_name(reviewed_df, ["Location", "location"])
    suggested_col = get_column_name(reviewed_df, ["suggested_allocation_column", "Suggested Allocation Column"])
...
def handle_status_filter(question: str, reviewed_df: pd.DataFrame) -> str:
    eligibility_col = get_column_name(reviewed_df, ["eligibility_status", "Eligibility Status"])
    suggested_col = get_column_name(reviewed_df, ["suggested_allocation_column", "Suggested Allocation Column"])
...
def handle_needs_documentation(reviewed_df: pd.DataFrame) -> str:
    status_col = get_column_name(reviewed_df, ["review_status", "Review Status"])
    eligibility_col = get_column_name(reviewed_df, ["eligibility_status", "Eligibility Status"])
    rationale_col = get_column_name(reviewed_df, ["rationale", "Rationale"])
    confidence_col = get_column_name(reviewed_df, ["confidence_score", "Confidence Score"])
...
def handle_documentation_rag(question: str) -> str:
    from boc_agent.rag.rag_answerer import RAGAnswerer
    answerer = RAGAnswerer()
    return answerer.answer(question)

def handle_tax_refusal(question: str, skill=None) -> str:
