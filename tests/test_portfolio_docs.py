...
def test_validate_documented_test_count():
    ...
    # Verify the current count "307" exists in relevant docs where test count is mentioned
    ...
    for filepath in docs_mentioning_count:
        with open(filepath, "r", encoding="utf-8") as f:
            content = f.read().lower()
            normalized_content = re.sub(r'\s+', ' ', content)
        assert ("307 tests" in normalized_content or
                "307 unit tests" in normalized_content or
                "307 unit and integration tests" in normalized_content or
                "307 passed" in normalized_content or
                "307-test" in normalized_content), f"Current test count 307 not mentioned in {filepath}"
